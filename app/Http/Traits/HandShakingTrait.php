    ...
    public function save_notification($data, $notification_type)
    {
        ...
        if ($notification_type == 'new_lead_added'){
            ...
            $notifications->data   = json_encode([
                ...
                "writer_url"=>url("/"),
                "sale_url"=>url("admin/leads"),
                "admin_url"=>url("admin/leads")                           
        ...
        if ($notification_type == 'new_order_added'){
            ...
            $notifications->data   = json_encode([
                ...
                "writer_url"=>url("admin/writers/task_details/{$saved_data->id}"),
                "sale_url"=>url("admin/orders"),
        ...
        if ($notification_type == 'task_assigned') {
            ...
            $notifications->data   = json_encode([
                "message"=>"{$this->full_name($saved_data->created_by)}, Assigned {$this->full_name($saved_data->user_id)} to ORDER-{$saved_data->sale_order_id} on {$created_at}",
                    "writer_url"=>url("admin/writers/task_details/{$saved_data->sale_order_id}"),
                                        "sale_url"=>url("admin/orders"),
                                        "admin_url"=>url("admin/orders")
            ]);
        ...
        if ($notification_type == 'order_status_change') {
            ...
            $notifications->data   =  json_encode([
                                        "message"=>"{$this->full_name($saved_data->created_by)}, Changed Status:{$saved_data->title} ORDER-{$saved_data->order} on {$created_at}",
                                         "writer_url"=>url("admin/writers/task_details/{$saved_data->order}"),
                                        "sale_url"=>url("admin/orders"),
                                        "admin_url"=>url("admin/orders")
                                    ]);
        ...
        if ($notification_type == 'user_task_status_changed') {
            ...
            //                                 "writer_url"=>url("admin/writers/task_details/{$saved_data->order_id}"),
            //                                 "sale_url"=>url("admin/orders"),
            //                                 "admin_url"=>url("admin/orders")
            //                             ]);
        ...
        if($notification_type == 'order_feedback_added'){
            ...
            if($saved_data->deadline){
                ...
                $notifications->data   = json_encode([
                    ...
                    "writer_url"=>url("admin/writers/task_details/{$saved_data->order_id}"),
                    "sale_url"=>url("admin/orders/order_full_details/{$saved_data->order_id}"),
                    "admin_url"=>url("admin/orders/order_full_details/{$saved_data->order_id}")
                ]);
            ...
            $notifications->data   = json_encode([
                    "message"=>"{$this->full_name($saved_data->sender_id)}, sent new message on ORDER-{$saved_data->order_id}",
                    "writer_url"=>url("admin/messages?order_id={$saved_data->order_id}"),
                    "sale_url"=>url("admin/messages?order_id={$saved_data->order_id}"),
                    "admin_url"=>url("admin/messages?order_id={$saved_data->order_id}"),
            ]);
