...
class HomeController extends Controller
{
        ...
        $task = $this->task_details();
            ...
            if(Auth::user()->roles[0]->name == 'Sale Agent' || Auth::user()->roles[0]->name =='Sale Manager' ||  Auth::user()->roles[0]->name =='Admin' ){
                ...
                if(Auth::user()->roles[0]->name != 'Admin'){
                     ...
                     $result['month_sale'] = number_format($month_result->sum('amount'),2);

                     $result['month_paid'] = number_format($month_result->sum('amount_received'),2);
    ...
    public function todays_leads(){
        ...
        if(Auth::user()->roles[0]->name == 'Admin'){
            ...
            })->orderBy('leads_count', 'desc')->paginate(5);
