            ...
            <div class="notify-menu" style="max-height:300px;overflow:auto;max-width:400px;">
               ...
               <div class="pull-right" v-if="message_loading">
                ...
                <div v-if="result" v-for="(notify, index) in result.result" :key="index">
                    <a v-if="role == 'Sale Agent' || role =='Sale Manager'" :href="notify.data.sale_url"
                    ...
                    <a v-else-if="role == 'Writer' || role =='Writer Manager'" :href="notify.data.writer_url"
                        class="dropdown-item border-bottom d-flex ps-4">
                        <div>
                            <span class="fs-13">{{ notify.data.message }}.</span>
                            <div class="small text-muted">{{ moment(notify.created_at).format("Y-M-D H:m") }}</div>
                    ...
                    <a v-else-if="role == 'Admin'" :href="notify.data.admin_url"
                        class="dropdown-item border-bottom d-flex ps-4">
                        <div>
                            <span class="fs-13">{{ notify.data.message }}.</span>
                            <div class="small text-muted">{{ moment(notify.created_at).format("Y-M-D H:m") }}</div>
                        ...
                        <!-- <span class="badge fs-10 bg-danger br-1 ms-auto" style="cursor:pointer" @click="clear_notifications_single(notify.id)">Clear</span> -->
                        <span class="badge fs-10 bg-danger br-1 ms-auto" style="cursor:pointer"
                    @click="clear_notifications_single(notify.id)">Clear</span>
                   </div>
...
export default {
  ...
  async clear_notifications_single(ele=null) {
     await axios.get(this.$hostname + "notifications/notify_notification/"+ele).then((response) => {
                    if (response.data.status == 'success') {
