            ...
            <div class="form-group mb-0">
              <!-- <input name="files" @change="processFileDocumentUpload($event)" type="file" id="task_documents_uploadd"
                accept=".xlsx,.xls,image/*,.doc,audio/*,.docx,video/*,.ppt,.pptx,.txt,.pdf" multiple /> -->
                	<vue-dropzone ref="myVueDropzone" id="dropzone" :options="dropzoneOptions" @vdropzone-complete="afterUploadComplete" @vdropzone-removed-file="removeFile"></vue-dropzone>
            </div>
          </div>
            ...
            <tbody v-if="role == 'Sale Manager' || role == 'Sale Agent'">
             ...
             <tbody v-else>
              <tr v-for="doc in writer_documetns" :key="doc.sale_document_id" >
                ...
                <td>{{ moment(doc.created_at).format("Y-M-D H:m")  }}</td>
                             <!-- <td>{{doc.created_at}}</td> -->
                <td><i @click="delete_document(doc.sale_document_id)" style="cursor:pointer" class="fa fa-trash"></i></td>
              </tr>
            </tbody>
          </table> 
...
//  components:{
  ...
  // },
    ...
    async delete_document(assigned_id) {
      const __this = this;
      swal({
        title: "Are you sure?",
        text: 'Click on yes, continue then document will deleted permenantly',
        type: "info",
        showCancelButton: true,
        confirmButtonColor: "#DD6B55",
        confirmButtonText: "Yes, Continue it!",
        ...
        function (isConfirm) {
          ...
          if (isConfirm) {
            ...
            $.ajax({
              ...
              url: main_url + '/writers/delete_doc/'+assigned_id,
              ...
              success: function (response) {
                ...
                if (response.status == 'success') {
                  ...
                  swal('Info!', response.message, response.status);
